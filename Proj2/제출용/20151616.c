...
{
		int i=0, j=0,opcode=0,niflag=0,xbpeflag=0,disp=0, format=0,errorflag=0;
		int line=0, loc=0, bloc=0, byte=0,reg1=0,reg2=0;
		int len=0;
...
{
		FILE *lstfp, *objfp;
		int wnum=0,loc=0,line=0,start=0,cflag=0,len=0,modcnt=0,endrecord=0,endflag=0,i=0;
		...
		sscanf(buf1,"%d %X %s %s %X",&line,&loc,subr,iname,&start);
		rewind(lstfp);
		if(!strcmp(iname,"START"))
		{
		...
		{
				fprintf(objfp,"H      000000%X\nT000000",endloc-start);
		}
		cflag=0;
